...
// EventIterator is used to iterate collection events
type EventIterator[T interface{}] struct {
	Iterator driver.EventIterator
	err      error
}

func (it *EventIterator[T]) Next(event *Event[T]) bool {
	if it.err != nil {
		return false
	}
...
// Err returns nil if iteration was successful,
// otherwise return error details
func (it *EventIterator[T]) Err() error {
	if it.Iterator.Err() != nil {
		return it.Iterator.Err()
	}
	return it.err
