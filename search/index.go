...
func (c *Index[T]) Drop(ctx context.Context) error {
	return getSearch(ctx, c.SearchClient).DeleteIndex(ctx, c.Name)
}
...
func (c *Index[T]) Get(ctx context.Context, IDs []string) ([]IndexDoc[T], error) {
	...
	for _, v := range resp {
		...
		if err = populateIndexDocMetadata(doc, v); err != nil {
			return nil, err
		}
