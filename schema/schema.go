...
var (
	tigrisPath      = "github.com/tigrisdata/tigris-client-go/tigris"
...
func traverseFields(prefix string, t reflect.Type, pk map[string]int, nFields *int) (map[string]Field, bool, error) {
	var embModel bool
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if _, ok := pk[prefix+fName]; ok && !isPrimaryKeyType(f.Type) {
			return nil, false, fmt.Errorf("type is not supported for the key: %s", field.Type.Name())
		}

		if f.AutoGenerate && !isPrimaryKeyType(f.Type) {
			return nil, false, fmt.Errorf("type cannot be autogenerated: %s", field.Type.Name())
		}
		...
		// only detect embedded tigris.Model at the top level
		if tp.Kind() == reflect.Struct && tp.Name() == tigrisModelType && tp.PkgPath() == tigrisPath && field.Anonymous {
			embModel = true
			continue
		}

		if tp.Kind() == reflect.Struct && f.Format != formatDateTime {
			f.Fields, _, err = traverseFields(prefix+fName, tp, nil, nFields)
			if err != nil {
				return nil, false, err
			}
		} else if f.Type == typeArray {
			if field.Type.Elem().Kind() == reflect.Struct {
				fields, _, err := traverseFields(prefix+fName, tp.Elem(), nil, nFields)
				if err != nil {
					return nil, false, err
				}
				f.Items = &Field{
