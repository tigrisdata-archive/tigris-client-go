...
func translateType(t reflect.Type) ([]string, string, error) {
	tp, nullable, format, err := translateTypeLow(t)
	if err != nil {
		return nil, "", err
	}
	if nullable {
		return []string{tp, null}, format, nil
	}
	return []string{tp}, format, nil
}

func translateTypeLow(t reflect.Type) (string, bool, string, error) {
	nullable := false
	if t.Kind() == reflect.Ptr {
		t = t.Elem()
		nullable = true
	...
	case reflect.Struct:
		if t.PkgPath() == "time" && t.Name() == "Time" {
			return typeString, nullable, formatDateTime, nil
		}
		return typeObject, nullable, "", nil
	case reflect.Array:
		if t.Elem().Kind() == reflect.Uint8 {
			if t.PkgPath() == "github.com/google/uuid" && t.Name() == "UUID" {
				return typeString, nullable, formatUUID, nil
			}
			return typeString, nullable, formatByte, nil
		}
		return typeArray, nullable, "", nil
	case reflect.Slice:
		if t.Elem().Kind() == reflect.Uint8 {
			return typeString, nullable, formatByte, nil
		}
		return typeArray, nullable, "", nil
	case reflect.Int32:
		return typeInteger, nullable, formatInt32, nil
	case reflect.Int64:
		return typeInteger, nullable, "", nil
	case reflect.String:
		return typeString, nullable, "", nil
	case reflect.Float32, reflect.Float64:
		return typeNumber, nullable, "", nil
	case reflect.Bool:
		return typeBoolean, nullable, "", nil
	case reflect.Map:
		return typeObject, nullable, "", nil
	case reflect.Int:
		var a int
...
func traverseFields(prefix string, t reflect.Type, fields map[string]*Field, pk map[string]int, nFields *int) error {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if _, ok := pk[prefix+fName]; ok && !isPrimaryKeyType(f.Type[0]) {
			return fmt.Errorf("type is not supported for the key: %s", field.Type.Name())
		}

		if f.AutoGenerate && !isPrimaryKeyType(f.Type[0]) {
		...
		if tp.Kind() == reflect.Struct && f.Format != formatDateTime {
			...
			if field.Type.Elem().Kind() == reflect.Struct {
				...
				if nullable {
					f.Items.Type = append(f.Items.Type, null)
				}
...
func fromCollectionModel(model interface{}) (*Schema, error) {
	...
	if len(pk) == 0 {
		...
		if p, ok = sch.Fields["Id"]; ok {
			...
			sch.Fields[id] = &Field{Type: []string{typeString}, Format: formatUUID, AutoGenerate: true}
			p = sch.Fields[id]
		}
		if !isPrimaryKeyType(p.Type[0]) {
			return nil, fmt.Errorf("type is not supported for the key: %s", p.Type[0])
		}
		p.AutoGenerate = true
