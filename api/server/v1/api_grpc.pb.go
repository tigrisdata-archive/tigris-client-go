...
type tigrisEventsClient struct {
	grpc.ClientStream
}

func (x *tigrisEventsClient) Recv() (*EventsResponse, error) {
...
func _Tigris_Events_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(EventsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(TigrisServer).Events(m, &tigrisEventsServer{stream})
}
...
type tigrisEventsServer struct {
	grpc.ServerStream
}

func (x *tigrisEventsServer) Send(m *EventsResponse) error {
	return x.ServerStream.SendMsg(m)
}
