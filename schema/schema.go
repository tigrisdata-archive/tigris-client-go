...
func translateType(t reflect.Type, f *Field) error {
	...
	switch t.Kind() {
	case reflect.Struct:
		f.Type.Set(typeObject)

		if t.PkgPath() == "time" && t.Name() == "Time" {
			f.Type.Set(typeString)
			f.Format = formatDateTime
		}
	case reflect.Array:
		f.Type.Set(typeArray)
		f.MaxItems = t.Len()
	...
	case reflect.Float32, reflect.Float64:
		f.Type.Set(typeNumber)
	case reflect.Bool:
		f.Type.Set(typeBoolean)
	case reflect.Map:
		f.Type.Set(typeObject)
...
func traverseFields(prefix string, t reflect.Type, fields map[string]*Field, pk map[string]int, nFields *int) error {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if _, ok := pk[prefix+fName]; ok && !isPrimaryKeyType(f.Type.First()) {
			return fmt.Errorf("type is not supported for the key: %s", field.Type.Name())
		}

		if f.AutoGenerate && !isPrimaryKeyType(f.Type.First()) {
		...
		if tp.Kind() == reflect.Struct && f.Format != formatDateTime {
			...
			f.Required = setRequired(f.Fields)
		} else if f.Type.First() == typeArray {
...
func fromCollectionModel(schemaVersion int, model any, typ string) (*Schema, error) {
	...
	if len(pk) == 0 && typ == Documents {
		...
		if p, ok = sch.Fields["Id"]; ok {
			...
			sch.Fields[id] = &Field{Type: NewMultiType(typeString), Format: formatUUID, AutoGenerate: true}
			p = sch.Fields[id]
		}

		if !isPrimaryKeyType(p.Type.First()) {
			return nil, fmt.Errorf("type is not supported for the key: %s", p.Type.First())
		}
